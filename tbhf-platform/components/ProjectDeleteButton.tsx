...
import { deleteProject } from "@/app/(app)/volunteer/actions";
...
export default function ProjectDeleteButton({ projectId }: { projectId: string }) {
  ...
  return (
    <span style={{ display: "inline-flex", gap: 8, alignItems: "center" }}>
      ...
      <button type="button" disabled={pending} onClick={() => start(async () => { await deleteProject(projectId); })} style={btn(true)}>
