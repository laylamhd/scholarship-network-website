...
import { addComment, toggleLike, deleteGroupPost } from "@/app/(app)/groups/feed-actions";
...
export default function PostCard({ post, groupId, canManage = false }: { post: FeedPost; groupId: string; canManage?: boolean }) {
  ...
  function onDelete() {
    start(async () => {
      const res = await deleteGroupPost(groupId, post.id);
  ...
  function onLike() {
    ...
    start(async () => {
      const res = await toggleLike(groupId, post.id);
  ...
  function submitComment() {
    ...
    start(async () => {
      const res = await addComment(groupId, post.id, text);
