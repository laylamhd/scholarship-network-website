...
export default function StoryDeleteButton({ storyId }: { storyId: string }) {
  const [confirm, setConfirm] = useState(false);
  const [pending, start] = useTransition();

  if (!confirm) {
    return (
      <button type="button" onClick={() => setConfirm(true)} style={btn(false)}>
        Delete
      </button>
    );
  }
  return (
    <span style={{ display: "inline-flex", gap: 8, alignItems: "center" }}>
      ...
      <button type="button" disabled={pending} onClick={() => start(async () => { await deleteStory(storyId); })} style={btn(true)}>
        {pending ? "Deleting…" : "Yes, delete"}
      </button>
      <button type="button" onClick={() => setConfirm(false)} style={{ ...btn(false), color: colors.inkMuted }}>Cancel</button>
    </span>
  );
}
